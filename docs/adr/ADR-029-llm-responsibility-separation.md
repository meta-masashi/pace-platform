...
## 補足: パイプライン-コンディショニングエンジン連携境界

### 重複メトリクス

PACE Platform には 2 つの計算パスが存在する:

| メトリクス | v6 パイプライン (`lib/engine/v6/`) | コンディショニングエンジン (`lib/conditioning/engine.ts`) |
|-----------|------------------------------|----------------------------------------------|
| Fitness EWMA | Node 2 で ACWR の慢性負荷として使用 | 42日スパン（α≈0.0465）で独立計算 |
| Fatigue EWMA | Node 2 で ACWR の急性負荷として使用 | 7日スパン（α=0.25）で独立計算 |
| ACWR | `adaptACWR()` 経由で conditioning-adapter から取得 | `calculateACWR()` で直接計算 |
| Readiness | Node 2: 複合スコア（ACWR 40% + Wellness 40% + 20%） | `calculateReadiness()`: Banister FFM ベース |
| HRV ペナルティ | 未実装（v6 パイプラインでは非対象） | Pro プランで `applyHRVPenalty()` を適用 |

### 設計上の意図

- **コンディショニングエンジン**: 日次チェックイン時の即時フィードバック用（EWMA + Readiness）。選手向け UI 表示。
- **v6 パイプライン**: フルベイズ推論パイプライン（P1-P5 優先階層判定）。スタッフ向け高度分析。
- 両パスの ACWR 値は `conditioning-adapter.ts` 経由で統一可能だが、Readiness の定義が異なる点に注意。
- 将来、Readiness 定義を統合する場合は、コンディショニングエンジンの Banister FFM を v6 パイプラインに移植する（逆は不可 — パイプラインの複合スコアは簡易版）。

### 守るべきルール

1. **両パスとも LLM フリー**: A2 原則は v6 パイプラインだけでなくコンディショニングエンジンにも適用される
2. **メトリクスの二重計算は許容**: 各パスが独立して正しく動作することの方が、DRY 原則より優先
3. **ACWR の一貫性**: `conditioning-adapter.ts` を正とし、v6 パイプラインから直接 ACWR を再計算しない
