...
export async function POST(
  request: Request,
): Promise<NextResponse<SimulateResponse | ErrorResponse>> {
  try {
    ...
    if (!athleteId || !validateUUID(athleteId)) {
      return NextResponse.json(
        { success: false, error: 'athleteId が不正です。' },
        { status: 400 },
      );
    }
    ...
    if (athleteError || !athlete) {
      return NextResponse.json(
        { success: false, error: 'アスリートが見つかりません。' },
        { status: 403 },
      );
    }
    ...
    console.error('[v6/simulate] エラー:', err);
    return NextResponse.json(
