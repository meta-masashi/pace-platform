...
export async function POST(
  ...
  try {
    ...
    if (!body.athleteId || typeof body.athleteId !== "string") {
      return NextResponse.json(
        { success: false, error: "athleteId が必要です。" },
        { status: 400 }
      );
    }
...
JSONのみを出力してください。説明文やマークダウンは不要です。`);
    ...
    if (err instanceof Error) {
      if (err.message === "RATE_LIMIT_EXCEEDED") {
        return NextResponse.json(
          { success: false, error: "APIリクエストの上限に達しました。しばらく待ってから再試行してください。" },
          { status: 429 }
        );
      }
      if (err.message === "GEMINI_EXHAUSTED") {
        return NextResponse.json(
          { success: false, error: "AI生成に失敗しました。しばらく待ってから再試行してください。" },
          { status: 503 }
        );
      }
      if (err.message === "GUARDRAIL_VIOLATION") {
        return NextResponse.json(
          { success: false, error: "AI出力が安全基準に違反しました。手動で入力してください。" },
          { status: 422 }
        );
      }
    }

    console.error("[soap:generate] 予期しないエラー:", err);
    return NextResponse.json(
