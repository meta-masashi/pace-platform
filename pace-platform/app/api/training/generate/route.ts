...
export async function POST(request: Request) {
  try {
    ...
    if (insertError || !workout) {
      console.error('[training/generate] 保存エラー:', insertError);
      return NextResponse.json(
    ...
    console.error('[training/generate] 予期しないエラー:', err);

    // Gemini 固有エラーのハンドリング
    if (err instanceof Error) {
      if (err.message === 'GEMINI_EXHAUSTED') {
        return NextResponse.json(
          { error: 'AI サービスが一時的に利用できません。しばらくしてから再試行してください。' },
          { status: 503 },
        );
      }
      if (err.message === 'RATE_LIMIT_EXCEEDED') {
        return NextResponse.json(
          { error: 'API 呼び出し上限に達しました。しばらくしてから再試行してください。' },
          { status: 429 },
        );
      }
    }
