...
export const POST = withApiHandler(async (req, ctx) => {
  ...
  if (!access.allowed) {
    throw new ApiError(403, 'この機能は Pro プラン以上で利用できます。');
  }
  ...
  if (!body.athleteId || !body.message) {
    throw new ApiError(400, 'athleteId と message は必須です。');
  }
  ...
  if (body.finalize) {
    ...
    if (!approvalRoles.includes(staff.role as string)) {
      throw new ApiError(403, 'メニュー確定にはAT/PT/master権限が必要です。');
    }
  }
...
`;
          ...
          for (const exercise of phase.exercises ?? []) {
            for (const tag of exercise.tags ?? []) {
              if (
                ...
                ctx.log.warn(
                  `禁忌タグ違反検出: exercise="${exercise.name}" tag="${tag}" — メニューを無効化`,
                );
  ...
  if (body.finalize) {
    ...
    if (!rehabSessionId) {
      throw new ApiError(400, 'メニュー確定にはセッションIDが必要です。');
    }
    ...
    if (currentSession?.approved_at) {
      throw new ApiError(409, 'このセッションは既に承認済みです。');
    }

    const finalizeMenu = menu ?? (currentSession?.menu_json as Record<string, unknown> | null);
    if (!finalizeMenu) {
      throw new ApiError(400, '確定するメニューがありません。AIにメニューを提案させてください。');
    }
    ...
    if (currentHardLock) {
      throw new ApiError(403, 'Hard Lock が有効です。医師の承認後にロックを解除してから確定してください。');
    }
  ...
  if (rehabSessionId) {
    ...
    if (updateError) {
      ctx.log.warn('セッション更新失敗（並行書き込み競合の可能性）', { detail: updateError.message });
    }
  } else {
