...
export async function GET(request: Request) {
  try {
    ...
    if (athletesError) {
      console.error('[pipeline/team:GET] 選手取得エラー:', athletesError);
      return NextResponse.json(
        { success: false, error: '選手一覧の取得に失敗しました。' },
        { status: 500 },
      );
    }
    ...
    if (tracesError) {
      console.error('[pipeline/team:GET] トレース取得エラー:', tracesError);
      return NextResponse.json(
        { success: false, error: 'トレースログの取得に失敗しました。' },
        { status: 500 },
      );
    }
    ...
    console.error('[pipeline/team:GET] 予期しないエラー:', err);
    return NextResponse.json(
