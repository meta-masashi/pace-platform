...
export async function POST(request: Request) {
  try {
    ...
    if (!body.teamId || !validateUUID(body.teamId)) {
      return NextResponse.json(
        { success: false, error: 'teamId は有効なUUIDである必要があります。' },
        { status: 400 },
      );
    }
    ...
    if (athleteError || !athletes || athletes.length === 0) {
      return NextResponse.json(
        { success: false, error: 'チームメンバーが見つかりません。' },
        { status: 404 },
      );
    }
    ...
    if (traceError) {
      console.error('[team-anomaly] トレースログ取得エラー:', traceError);
      return NextResponse.json(
    ...
    console.error('[team-anomaly] 予期しないエラー:', err);
    return NextResponse.json(
