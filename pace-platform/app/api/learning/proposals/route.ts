...
export async function GET(
  ...
  try {
    ...
    if (!staff || staff.role !== "master") {
      return NextResponse.json(
        { success: false, error: "master ロールのスタッフのみアクセス可能です。" },
        { status: 403 }
      );
    }
    ...
    if (fetchError) {
      console.error("[learning:proposals] 取得エラー:", fetchError);
      return NextResponse.json(
    ...
    console.error("[learning:proposals] 予期しないエラー:", err);
    return NextResponse.json(
  ...
  try {
    ...
    if (!body.proposalId || !["approved", "rejected"].includes(body.action)) {
      return NextResponse.json(
        {
          success: false,
          error: "proposalId と action ('approved' | 'rejected') が必要です。",
        },
        { status: 400 }
      );
    }
    ...
    if (fetchError || !proposal) {
      return NextResponse.json(
        { success: false, error: "指定された提案が見つかりません。" },
        { status: 404 }
      );
    }
    ...
    if ((proposal.status as string) !== "pending") {
      return NextResponse.json(
        { success: false, error: "この提案は既にレビュー済みです。" },
        { status: 409 }
      );
    }
    ...
    if (updateError) {
      console.error("[learning:proposals] 更新エラー:", updateError);
      return NextResponse.json(
        { success: false, error: "提案の更新に失敗しました。" },
        { status: 500 }
      );
    }
    ...
    if (body.action === "approved") {
      ...
      if (nodeUpdateError) {
        console.error(
          `[learning:proposals] ノード更新失敗 ${nodeId}:`,
          nodeUpdateError
        );
      }
