...
function getLoad(date: string): number {
  // Use mock history or compute from schedule events
  if (LOAD_HISTORY[date] !== undefined) return LOAD_HISTORY[date];
  const ev = mockScheduleEvents.find(e => e.date === date);
  if (ev?.estimated_rpe && ev?.estimated_duration_min) return ev.estimated_rpe * ev.estimated_duration_min;
  return 0;
}

function computeACWR(baseDate: string): { acute: number; chronic: number; acwr: number } {
  const base = new Date(baseDate + "T00:00:00");
  let acute = 0;
  let chronic = 0;
  for (let i = 1; i <= 7; i++) {
    const d = new Date(base); d.setDate(base.getDate() - i);
    acute += getLoad(toYMD(d));
  }
  for (let i = 1; i <= 28; i++) {
    const d = new Date(base); d.setDate(base.getDate() - i);
    chronic += getLoad(toYMD(d));
  }
...
function buildWeeklyACWRChart(weekDates: Date[]): Array<{ day: string; acwr: number; projected: boolean }> {
  return weekDates.map((d, i) => {
    const ymd = toYMD(d);
    const today = "2026-03-21";
    const isToday = ymd === today;
    const isFuture = ymd > today;
    // For future dates, include this day's planned load in acute window
    if (isFuture) {
      const ev = mockScheduleEvents.find(e => e.date === ymd);
      const dayLoad = ev?.estimated_rpe && ev?.estimated_duration_min
        ? ev.estimated_rpe * ev.estimated_duration_min : 0;
      // Inject this load into LOAD_HISTORY temporarily
      const saved = LOAD_HISTORY[ymd];
      LOAD_HISTORY[ymd] = dayLoad;
      const result = computeACWR(ymd);
...
export default function SchedulePage() {
  const [anchor, setAnchor] = useState(new Date("2026-03-21"));
  ...
  const today = "2026-03-21";
  const weeklyACWR = buildWeeklyACWRChart(weekDates);
  const todayACWR = computeACWR(today);
  ...
  const attendance = selected ? mockAttendance.filter(a => a.event_id === selected.id) : [];
  const presentCount = attendance.filter(a => a.status === "present" || a.status === "late").length;
  const rpeEntries = attendance.filter(a => a.rpe_reported != null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={() => setAnchor(new Date("2026-03-21"))}
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="grid grid-cols-7 gap-1">
            {weekDates.map((date, i) => {
              ...
              const dayEvents = mockScheduleEvents.filter(e => e.date === ymd);
              const isToday = ymd === today;
              const isSat = i === 5;
              const isSun = i === 6;
                ...
                <div key={ymd} className="min-h-[180px]">
                  ...
                  <div className="space-y-1 px-0.5">
                    ...
                    {dayEvents.map(ev => {
                      ...
                      const att = mockAttendance.filter(a => a.event_id === ev.id);
            ...
            <Card>
              ...
              <CardContent className="space-y-4 pt-0">
                ...
                {selected.estimated_rpe && selected.estimated_duration_min && (() => {
                  ...
                  const base = computeACWR(selected.date);
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        ...
                        <span>{presentCount}/{attendance.length}名</span>
                    ...
                    <p className="text-xs text-gray-400 text-right mt-0.5">
                      出席率 {Math.round((presentCount / attendance.length) * 100)}%
                    </p>
                  </div>
                )}

                {attendance.length === 0 && selected.event_type !== "off" && (
                ...
                <div className="pt-1 border-t border-gray-100">
                  <p className="text-xs text-gray-400">
                    担当: {mockStaff.find(s => s.id === selected.created_by_staff_id)?.name ?? "—"}
                  </p>
                </div>
              </CardContent>
            </Card>
